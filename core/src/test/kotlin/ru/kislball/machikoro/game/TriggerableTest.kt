...
class TriggerableTest {
  ...
  fun `apply invokes effect only when trigger matches`() {
    ...
    val triggered = StubCard("cards.ranch", triggered = true, effect = effect)
    val notTriggered = StubCard("cards.bakery", triggered = false, effect = effect)
    val player = Player("p1")
    val game = Game(listOf(player))
    val step = game.nextStep()

    triggered.apply(step, player)
    notTriggered.apply(step, player)
